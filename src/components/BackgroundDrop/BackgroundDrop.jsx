...
function BackgroundDrop() {
    ...
    useEffect(() => {
        axios.get(`${tvOrSeries[oneOrTwo]}/popular?language=en-US&page=1&api_key=${API_KEY}`).then((response) => {
            ...
            console.log(response.data.results[randNum].original_title);
            console.log(response.data.results[randNum].name);
            console.log(response.data.results[randNum].overview);
        })
    }, [])
    return (
        <div className='backgroundDrop'>
            ...
            <div className="container-fluid">
                <BackdropText title={movie && movie.title} description={movie && movie.overview} movieDetails={movie && movie} />
